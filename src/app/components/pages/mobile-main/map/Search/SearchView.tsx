...
export const SearchView = () => {
  const { mapSearchView, setMapSearchView, mapCenter, setMapCenter } = useMobileMapStore();
  ...
  const search = async (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.code.toUpperCase() === 'ENTER') {
      ...
      const result = await mobileApi.naverApiG(event.currentTarget.value, mapCenter[1], mapCenter[0]);
      setMapCenter([result.addresses[0].y, result.addresses[0].x]);
  ...
  return (
    <SearchViewMain style={{ transform: transformValue }}>
      <SearchHeader>
        <Input placeholder="검색어를 입력해 주세요." value={searchWord}
               onChange={(event) => setSearchWord(event.currentTarget.value)} onKeyDown={(event) => search(event)} />
        <Button>
