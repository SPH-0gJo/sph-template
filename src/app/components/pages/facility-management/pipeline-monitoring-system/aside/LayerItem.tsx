...
export const LayerItem = (props: LayerItemProps) => {
  ...
  const LayerDivList = layerList.map((layer) => {
    ...
    return (
      <Layer key={`mn_layer_${layer}`}>
        <CheckBox id={layer} type='checkbox' checked={layerGroupsList.includes(layer)} onChange={layerClick} />
  ...
  return (
    <LayerItemWrapper>
      <ItemHeader>
        <ItemTitle>
          <CheckBox id={`title_${id}`} type='checkbox' checked={allCheck} onChange={allCheckChange} />
