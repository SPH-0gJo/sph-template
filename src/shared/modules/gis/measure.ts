...
import { addDistanceLayers, measureDistanceAction, removeDistanceLayers } from 'shared/modules/gis/measure.distance';

export function measureControl(map: AppMap) {
  const measureType = useMapMeasureStore.getState().measureType;
  const distanceSource = useMapMeasureStore.getState().distanceSource;
